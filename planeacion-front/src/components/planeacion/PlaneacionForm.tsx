...
export default function PlaneacionForm({
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => setSavedPulse(false), 900);
    ...
    (async () => {
      try {
        ...
        if (res.status === 404) {
          ...
          setSectionProgress(
            computeSectionProgress(form.getValues() as PlaneacionType)
          );

          toast.message(
            "No se encontró una planeación previa. Crea una nueva para comenzar."
          );
          return;
        }
  ...
  function buildPayload(values: PlaneacionType, opts?: { finalizar?: boolean }) {
    const payload: any = {
      ...
      // @ts-expect-error
      org_estrategia:
        (values as any).organizacion?.organizacion?.estrategia ??
        (values as any).organizacion?.estrategia ??
        "",
      // @ts-expect-error
