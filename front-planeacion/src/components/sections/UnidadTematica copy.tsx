    ...
    <div className="border rounded-xl p-3 space-y-3">
      <div className="grid sm:grid-cols-3 gap-2 items-end">
        <div>
          ...
          <Input type="number" value={idx + 1} readOnly tabIndex={-1} />
          <input type="hidden" {...register(`${path}.numero_sesion` as const, { valueAsNumber: true })} />
        </div>

        <div className="sm:col-span-2">
          <Label>Temas y subtemas</Label>
          <Input placeholder="Tema — Subtema…" {...register(`${path}.temas_subtemas` as const)} />
        </div>
      </div>
      ...
      <div className="grid sm:grid-cols-2 gap-2">
        ...
        <div>
          <Label>Valor (%) {restante === 0 ? "(sin restante)" : `(restante: ${restante}%)`}</Label>
          <Input
            type="number"
            min={0}
            step="1"
